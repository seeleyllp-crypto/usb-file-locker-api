...
<script>
const $=id=>document.getElementById(id);let mode="login";let session=sessionStorage.getItem("vaultlink_account_session")||"";let sessionExpires=sessionStorage.getItem("vaultlink_account_expires")||"";let current=null;let activityItems=[];let availabilityTimer=null;let refreshBusy=false;
...
function signedOut(note=""){session="";sessionExpires="";current=null;activityItems=[];sessionStorage.removeItem("vaultlink_account_session");sessionStorage.removeItem("vaultlink_account_expires");$("authView").classList.remove("hidden");$("accountView").classList.add("hidden");$("logout").classList.add("hidden");$("topStatus").textContent="Signed out";clearSensitiveFields();if(note)message("authMessage",note,"good")}
...
function downloadJson(filename,data){const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=filename;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}
...
$("copyAccountId").onclick=async()=>{if(!current?.account_id)return;await navigator.clipboard.writeText(current.account_id);$("copyAccountId").textContent="COPIED";setTimeout(()=>$("copyAccountId").textContent="COPY ACCOUNT ID",1200)};
for(const id of["username","password","confirm"]){$(id).addEventListener("keydown",event=>{if(event.key==="Enter")$("submit").click()})}for(const id of["currentPassword","newPassword"]){$(id).addEventListener("keydown",event=>{if(event.key==="Enter")$("changePassword").click()})}for(const id of["renameUsername","renamePassword"]){$(id).addEventListener("keydown",event=>{if(event.key==="Enter")$("changeUsername").click()})}
