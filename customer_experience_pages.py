...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; letter-spacing:0; }
    ...
    .metrics { display:grid; grid-template-columns:repeat(7,minmax(120px,1fr)); margin-top:16px; border:1px solid var(--line); }
...
<body>
  ...
  <main>
    ...
    <section class="signin">
      ...
      <div><label for="appVersion">Installed version, optional</label><input id="appVersion" maxlength="80" autocomplete="off" spellcheck="false" placeholder="2026.07.14.1"></div>
    ...
    <div id="workspace" hidden>
      <div class="toolbar"><button id="copy" class="blue" type="button">COPY SAFE SUMMARY</button><button id="export" type="button">EXPORT SAFE JSON</button><button id="resetProgress" type="button">RESET CHECKLIST</button></div>
  ...
  <script>
    ...
    const state = { payload:null, completed:new Set() };
    ...
    function safeExport() {
      ...
      return { exported_at_utc:new Date().toISOString(), workspace_schema_version:state.payload.workspace_schema_version, summary:state.payload.summary, checkup:state.payload.checkup, action_center:state.payload.action_center, timeline:state.payload.timeline, rank_tools:state.payload.rank_tools, upgrade_options:state.payload.upgrade_options, completed_action_ids:[...state.completed].sort(), privacy_notice:state.payload.privacy_notice };
    }
    ...
    function renderActions(data) {
      ...
      items.forEach((item)=>{ const row=document.createElement("div"); row.className=`action ${item.when}`; const check=document.createElement("input"); check.type="checkbox"; check.checked=state.completed.has(item.id); check.addEventListener("change",()=>{ if(check.checked)state.completed.add(item.id);else state.completed.delete(item.id); updateProgress(items.length); }); const body=document.createElement("div"); addText(body,"h3",item.title); addText(body,"p",item.detail); row.append(check,body); addText(row,"div",item.when,"when"); root.append(row); });
    ...
    function renderTools(data) {
      ...
      tools.forEach((item)=>{ const card=document.createElement("article"); card.className="tool"; addText(card,"div",`Rank ${item.rank} | ${item.category} | ${item.estimated_minutes} min`,"eyebrow"); addText(card,"h3",item.name); addText(card,"p",item.summary); const list=document.createElement("ul"); (item.checklist||[]).forEach(step=>addText(list,"li",step)); card.append(list); root.append(card); });
    }
    ...
    function render(data) { state.payload=data; state.completed=new Set(); renderMetrics(data); renderActions(data); renderTools(data); renderTimeline(data); renderUpgrades(data); renderLinks(data); $("workspace").hidden=false; }
    ...
    async function copySummary() { const data=safeExport(); if(!data)return; const lines=["VaultLink Customer Workspace",`Status: ${data.summary.status}`,`Rank: ${data.summary.plan.rank} - ${data.summary.plan.name}`,`Seats: ${data.summary.device_usage.active} of ${data.summary.device_usage.maximum}`,`Service: ${data.summary.service_status.mode}`,`Latest desktop: ${data.summary.release.latest_version||"Not published"}`,`Actions needing attention: ${data.checkup.attention_count}`]; try{await navigator.clipboard.writeText(lines.join("\n"));setStatus("Privacy-safe summary copied.","good");}catch(_){setStatus("Browser clipboard access was blocked.","bad");} }
    function exportJson() { const data=safeExport(); if(!data)return; const blob=new Blob([JSON.stringify(data,null,2)],{type:"application/json"}); const url=URL.createObjectURL(blob); const link=document.createElement("a"); link.href=url; link.download="vaultlink-customer-workspace.json"; document.body.append(link); link.click(); link.remove(); setTimeout(()=>URL.revokeObjectURL(url),1000); setStatus("Privacy-safe workspace exported.","good"); }
    ...
    $("load").addEventListener("click",loadWorkspace); $("clear").addEventListener("click",clearAll); $("copy").addEventListener("click",copySummary); $("export").addEventListener("click",exportJson); $("resetProgress").addEventListener("click",()=>{state.completed=new Set(); if(state.payload)renderActions(state.payload);}); $("licenseKey").addEventListener("keydown",event=>{if(event.key==="Enter")loadWorkspace();});
  </script>
</body>
</html>'''
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; letter-spacing:0; }
    ...
    .metrics { display:grid; grid-template-columns:repeat(6,minmax(130px,1fr)); margin-top:16px; border:1px solid var(--line); }
    .metric { min-width:0; padding:15px; border-right:1px solid var(--line); background:var(--band); }
...
<body>
  ...
  <main>
    ...
    <div id="console" hidden>
      <div class="toolbar"><button id="refresh">REFRESH</button><button id="export">EXPORT SAFE JSON</button><button id="csv">EXPORT RANK CSV</button></div>
      <div id="metrics" class="metrics"></div>
  ...
  <script>
    ...
    function render(data){state.payload=data;const metrics=$("metrics");metrics.replaceChildren();[["Active licenses",data.metrics.active_licenses],["Active devices",data.metrics.active_devices],["Release adoption",`${data.metrics.release_adoption_percent}%`],["Support needs action",data.metrics.support_needs_action],["Active announcements",data.metrics.active_announcements],["Shop links",`${data.metrics.shop_links_live} / ${data.metrics.shop_links_total}`]].forEach(row=>metrics.append(metric(...row)));cards("actions",data.actions,item=>card(item.title,item.detail,item.state,item.category));cards("ranks",data.rank_coverage,item=>card(`Rank ${item.rank} - ${item.name}`,`${item.licenses} license(s) | ${item.entitlement_count} cumulative entitlement(s)`,item.licenses?"good":"info",item.price_label));cards("surfaces",data.customer_surfaces,item=>card(item.label,`${item.path} | ${item.purpose}`,item.ready?"good":"check",item.ready?"ready":"check"));const release=data.release;const releaseReady=Boolean(release.signed_release_ready);const storageReady=/^(\d+) of \1 persistent$/.test(data.storage_readiness);const releaseRows=[["Desktop release",release.desktop_version||"Not published",releaseReady?"good":"check"],["API version",release.api_version,"good"],["Signature",release.signature_check,release.signature_check==="passed"?"good":"action"],["Package hash",release.package_hash_check,release.package_hash_check==="passed"?"good":"action"],["Service",data.service_status.mode,data.service_status.mode==="normal"?"good":"check"],["Storage",data.storage_readiness,storageReady?"good":"action"]];cards("release",releaseRows.map(([title,detail,tone])=>({title,detail,tone})),item=>card(item.title,item.detail,item.tone));$("privacy").textContent=data.privacy_notice;$("console").hidden=false;setStatus("Customer experience console refreshed.","good");}
    ...
    $("connect").addEventListener("click",connect);$("clear").addEventListener("click",clear);$("refresh").addEventListener("click",load);$("export").addEventListener("click",exportJson);$("csv").addEventListener("click",exportCsv);$("token").addEventListener("keydown",event=>{if(event.key==="Enter")connect();});
  </script>
</body>
</html>'''
    return page.replace("__API_VERSION__", html.escape(str(api_version), quote=True))
