...
- Download Verification Center discovery for hash, local receipt integrity sealing, standalone receipt inspection, bounded receipt-folder audits with a single local review window, bounded row and review-ID consumption, cancellable search debounce, selection preservation, fixed four-level triage, temporary review marks, 100-step numeric undo, aggregate progress with an aggregate level breakdown, failure-first navigation, full pending navigation, file/ZIP structure review, prior receipt comparison, and fixed audit actions without receiving selected files, folders, names, search text, delayed-callback state, review IDs, undo history, progress, selected rows, navigation, guidance state, filter choices, sorting, local results, receipts, receipt keys, paths, archive entry names, hashes, signature details, Defender output, inspection reports, folder-audit reports, or comparison output automatically
