...
- Download Verification Center discovery for hash, local receipt integrity sealing, standalone receipt inspection, bounded receipt-folder audits with searchable local review, fixed four-level triage guidance, temporary review marks, hide-reviewed mode, progress counts, session reset, failure-first navigation, file/ZIP structure review, prior receipt comparison, and fixed audit actions without receiving selected files, folders, names, search text, review marks, progress, selected rows, navigation, guidance state, filter choices, sorting, local results, receipts, receipt keys, paths, archive entry names, hashes, signature details, Defender output, inspection reports, folder-audit reports, or comparison output automatically
