...
`GET` and `POST /api/v1/accounts/support` let a signed-in customer send and review encrypted-at-rest help requests without copying a full license key into the browser. `POST /api/v1/accounts/support/reply` appends a customer follow-up, `POST /api/v1/accounts/support/close` closes a request, and `POST /api/v1/accounts/support/read` clears the signed-in customer's unread-owner-reply count for one request. Follow-ups reopen resolved or closed requests, and both customer and owner views show the encrypted conversation history. Version 0.70.0 added privacy-safe status summaries, browser-local search and filters in both inboxes, and an account-scoped `POST /api/v1/accounts/support/read-all` action. Version 0.71.0 added server-derived waiting-on-owner, waiting-on-customer, and finished workflow counts; stable update/creation sorting; next-unread navigation; message counts; tab unread badges; and opt-in browser alerts that contain only an aggregate unread count. Version 0.72.0 added compact current-tab-only expandable conversations, expand-unread and collapse-all controls, fixed-field metadata-only customer and owner queue exports, and an owner-authenticated aggregate `POST /api/v1/admin/support-tickets/read-all` action. Version 0.73.0 added owner-only normal, high, and urgent priorities; calculated wait-age bands; priority and longest-wait sorting; 24-hour owner-action filters; aggregate priority and age counts; and version-two metadata-only queue exports. Version 0.74.0 added owner bulk priority changes for up to 100 currently filtered requests, fixed reply templates that only fill a draft until Save is pressed, customer ticket-ID copy controls, and one-ticket metadata-only safe receipts. Version 0.75.0 adds account-scoped current-tab drafts for new requests and follow-ups, character counters, clear-draft controls, and owner-only calculated response-target states with due-soon and overdue filters, target ordering, counts, and version-three metadata-only exports. Drafts are never uploaded until the customer presses Send and are cleared after a successful send or sign-out. Response targets are internal planning aids derived from priority and waiting time; they are not service-level agreements, guarantees, or customer-visible promises. Priority and response-target data are not returned to customers, waiting ages are calculated rather than stored, and older owner clients that omit priority preserve the current value. Bulk read and bulk priority actions return aggregate counts only. Customer bulk read cannot affect another account, and owner bulk actions require the admin header. Requests are linked to the authenticated account, creation and follow-up actions are rate-limited, and no files, logs, passwords, license keys, device identifiers, or USB secrets are attached automatically. The owner support inbox receives the account ID and current username for account-originated requests, while customer responses exclude private owner notes, owner-only priority, and response-target metadata.
...
Version 0.78.0 adds six fixed owner routing queues, category-derived route suggestions, an unassigned-work filter, queue-aware sorting and aggregate counts, protected bulk routing for up to 100 filtered requests, and a derived four-step customer progress tracker. Customers can filter requests by received, under-review, and waiting-for-you stages and see a fixed progress badge without receiving owner routing, suggestions, labels, notes, scores, or other private planning metadata. Bulk routing returns aggregate counts only. Customer receipt and queue exports include only the derived progress stage; owner exports include fixed routing metadata but continue to exclude customer text and identity.

Version 0.79.0 adds a server-derived four-step customer timeline, fixed next-action guidance, a My Action Needed view, action-first sorting, and aggregate customer action counts. Finished requests ask for a fixed resolution outcome only when one has not already been saved. The owner console adds six clickable queue-health controls with aggregate action-needed, overdue, due-soon, unread, urgent, high-attention, and oldest-wait metrics. Timeline and queue-health exports remain metadata-only: they do not add customer text, identity, owner notes, credentials, license keys, files, paths, PINs, or USB secrets.

Version 0.80.0 adds matching browser-local and server-enforced sensitive-information guards for new support requests, customer follow-ups, owner replies, and private owner notes. The fixed detector rejects private-key headers, Discord webhook secrets, VaultLink license/session/download tokens, bearer tokens, strongly labeled passwords and secrets, labeled PINs, Social Security numbers, and Luhn-valid payment-card numbers. The customer page shows a live privacy result and category-specific safe-writing guidance before sending. Rejections use a fixed message and never include, log, export, or store the detected text. This guard is a narrow accident-prevention layer, not a full data-loss-prevention system, and customers must still avoid entering private information.
...
- `GET /api/v1/accounts/support`
  - Authenticated account inbox with privacy-safe status and unread summaries.
- `POST /api/v1/accounts/support/read-all`
  - Marks all owner replies read only for the authenticated account and returns aggregate counts without ticket IDs or message text.
...
- `POST /api/v1/admin/support-tickets/action`
  - Admin-only priority, status, customer reply, and private owner-note update. Priority accepts only `normal`, `high`, or `urgent`.
- `POST /api/v1/admin/support-tickets/priority-bulk`
...
- `POST /api/v1/admin/support-tickets/triage`
  - Admin-only pin, one fixed routing queue, and up to three labels selected from the fixed owner-label list. It accepts no free-form queue or label text.
- `POST /api/v1/admin/support-tickets/route-bulk`
  - Admin-only fixed-queue routing for up to 100 prevalidated ticket IDs. It returns aggregate counts only and never returns ticket text or customer identity.
...
- `POST /api/v1/admin/support-tickets/read-all`
  - Admin-only aggregate mark-read action for every unread customer support message. It returns counts only and does not return ticket text.
